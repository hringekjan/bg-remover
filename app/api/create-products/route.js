...
const client_1 = require("@/lib/s3/client");
const client_2 = require("@/lib/carousel-api/client");
const job_store_1 = require("@/lib/dynamo/job-store");
const jwt_validator_1 = require("@/src/lib/auth/jwt-validator");
const client_3 = require("@/src/lib/credits/client");
/**
...
const ProductGroupSchema = zod_1.z.object({
    productId: zod_1.z.string().optional(), // Optional group identifier
    images: zod_1.z.array(ImageInputSchema).min(1).max(20),
...
const CreateProductsRequestSchema = zod_1.z.object({
    productGroups: zod_1.z.array(ProductGroupSchema).min(1).max(50),
    options: zod_1.z.object({
        outputFormat: zod_1.z.enum(['webp', 'png', 'jpeg']).default('webp'),
        quality: zod_1.z.number().min(1).max(100).default(90),
        removeBackground: zod_1.z.boolean().default(true),
        generateDescription: zod_1.z.boolean().default(false),
        languages: zod_1.z.array(zod_1.z.string()).default(['en']),
    }).optional(),
});
/**
 * Process a single image: bg removal + upload to S3
 */
async function processAndUploadImage(imageInput, tenant, productGroupId, outputFormat, quality, removeBackground) {
    const startTime = Date.now();
    try {
        // Process image (these functions throw on error)
        let result;
        if (imageInput.url) {
            result = await (0, image_processor_1.processImageFromUrl)(imageInput.url, {
                format: outputFormat,
                quality,
            });
        }
        else if (imageInput.base64) {
            result = await (0, image_processor_1.processImageFromBase64)(imageInput.base64, 'image/png', {
                format: outputFormat,
                quality,
            });
        }
        ...
        // Upload to S3
        const outputKey = (0, client_1.generateOutputKey)(tenant, productGroupId, outputFormat);
        const contentType = outputFormat === 'png' ? 'image/png' :
            outputFormat === 'webp' ? 'image/webp' : 'image/jpeg';
        const outputBucket = await (0, client_1.getOutputBucket)(tenant);
        const s3Url = await (0, client_1.uploadProcessedImage)(outputBucket, outputKey, result.outputBuffer, contentType, {
 ...
 */
async function processProductGroup(group, tenant, userId, options, index, timeoutMs) {
    const productGroupId = group.productId || `group-${index}-${(0, crypto_1.randomUUID)().substring(0, 8)}`;
    const startTime = Date.now();
    ...
    try {
        ...
        const processingPromise = Promise.all(group.images.map(img => processAndUploadImage(img, tenant, productGroupId, options.outputFormat, options.quality, options.removeBackground)));
        // Apply timeout if specified
        let imageProcessingResults;
        if (timeoutMs && timeoutMs > 0) {
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Group processing timeout')), timeoutMs));
        ...
        const { product, error } = await (0, client_2.createProductInCarouselApi)(tenant, userId, productData);
        if (error || !product) {
            return {
                productId: productGroupId,
                status: 'failed',
 ...
 */
const calculatePerGroupTimeout = (totalGroups, lambdaTimeout) => {
...
async function POST(request) {
    ...
    try {
        ...
        const authResult = await (0, jwt_validator_1.validateJWTFromEvent)({ headers: authHeaders }, undefined, {
        ...
        if (authResult.isValid && authResult.payload) {
            ...
            userTenantId = claims['custom:tenant'] ||
        ...
        if (creditsRequired && authResult.isValid && authResult.userId) {
            ...
            const creditResult = await (0, client_3.validateAndDebitCredits)(tenant, authResult.userId, totalImages, // 1 credit per image across all groups
            jobId, undefined // No single productId for multi-product batch
            );
        ...
        await (0, job_store_1.setJobStatus)(jobId, {
            jobId,
            status: 'processing',
            tenant,
            userId,
        ...
        const results = await processBatches(validatedRequest.productGroups, CONCURRENCY_LIMIT, (group, index) => processProductGroup(group, tenant, userId, options, index, perGroupTimeoutMs));
        ...
        if (creditsRequired && creditTransactionId && failedImageCount > 0 && authResult.userId) {
            ...
            const refundResult = await (0, client_3.refundCredits)(tenant, authResult.userId, failedImageCount, // Refund 1 credit per failed image
        ...
        if (creditsRequired && creditTransactionId && totalImages > 0) {
            ...
            try {
                ...
                const refundResult = await (0, client_3.refundCredits)(tenant, userId, totalImages, jobId, creditTransactionId);
                if (refundResult.success) {
